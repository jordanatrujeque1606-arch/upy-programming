...
try:
    if rol_actual == 'alumno':
        ...
        # Validación robusta para evitar caídas al convertir texto a número flotante
        entrada_cal = input("Nueva calificación: ").strip()
        try:
            nueva_cal = float(entrada_cal)
        except ValueError:
            raise ValueError("La calificación ingresada debe ser un número válido (ej. 8.5).")
        
        if not (0.0 <= nueva_cal <= 10.0):
            raise ValueError("La calificación debe estar en un rango de 0.0 a 10.0.")
        ...
        if alumno_elegido in calificaciones and materia_elegida in materias:
            ...
            print("Error: El username del alumno o la materia ingresados no son válidos.")
    ...
    print(f"Error en las estructuras de datos (clave faltante): {e}")
except Exception as e:
    print(f"Ocurrió un error inesperado durante la ejecución del sistema: {e}")
